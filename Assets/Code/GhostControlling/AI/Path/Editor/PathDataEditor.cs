    ...
    private void OnSceneGUI()
    {
        ...
        if (pathData.path != null)
        {
            ...
            else if(pathData.path.vertexesCount == 1)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                Vertex vertex = pathData.path.GetVertexbyID(0);
                EditorGUILayout.LabelField("Type", GUILayout.Width(80));
                vertex.vertexType = (Vertex.VertexType)EditorGUILayout.EnumPopup(vertex.vertexType);
                EditorGUILayout.EndHorizontal();

                if (vertex.vertexType != Vertex.VertexType.None)
                {
                    vertex.ObjectID = EditorGUILayout.IntField("Player ID", vertex.ObjectID);
                }
            }
            else
            {
                ...
                Vertex vertex = pathData.path.GetVertexbyID(editedvertex);
                EditorGUILayout.LabelField("Type", GUILayout.Width(80));
                vertex.vertexType = (Vertex.VertexType)EditorGUILayout.EnumPopup(vertex.vertexType);
                EditorGUILayout.EndHorizontal();
                if (vertex.vertexType != Vertex.VertexType.None)
